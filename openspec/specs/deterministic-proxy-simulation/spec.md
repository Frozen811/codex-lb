# deterministic-proxy-simulation Specification

## Purpose
Proxy turn lifecycle regressions repeatedly come from timing-dependent interleavings (admission waits, upstream terminal events, downstream cancellation, retries, lease release ownership) that real-sleep tests cover only with scheduler jitter. This capability lets proxy lifecycle code accept clock and scheduler collaborators so tests can drive those interleavings under deterministic virtual time while production keeps real-time defaults.
## Requirements

- [x] `app/modules/proxy/helpers.py`: add `_USAGE_LIMIT_MESSAGE_MARKERS` beside
...
- [x] `app/core/balancer/types.py` + `helpers.py`: add `excludes_account: bool`
...
- [x] Cover the wordings upstream actually sends. `"The usage limit has been
...
- [x] Preserve client retry guidance across the reclassification. A code-less
...
- [x] Make exhaustion provable inside the request from the WALK's evidence, not
...
- [x] Terminal rendering must skip the probe for the two bounds whose answer the
...
- [x] `app/core/balancer/logic.py`: replace `candidates_remaining: int` with
  ...
  one release.
- [x] Record which bound ended a walk (non-retryable, exhausted pool, deadline,
  ceiling, progress failure). `failover_decision` returning `surface` for all of
  them is what makes the reorder unobservable today.
- [x] Carve the capacity-recovery re-admission out of the monotone-progress
...
- [x] New `app/modules/proxy/pool_terminal.py`:
...
- [x] `_service/streaming/retry.py`: convert the bounded
...
- [x] `_service/websocket/mixin.py` and `_service/compact.py`: same walk shape;
  pass `owner_bound` and `same_account_retry_available` on the websocket path,
  which omits them today.
- [x] `app/modules/proxy/service.py`: delete `_STREAM_MAX_ACCOUNT_ATTEMPTS`,
  `_WEBSOCKET_MAX_ACCOUNT_ATTEMPTS` and `_COMPACT_MAX_ACCOUNT_ATTEMPTS`.
- [x] Verify the per-account health write stays on the `failover_next` branch
  and is not duplicated by the walk.
- [x] `tests/unit/test_failover_foundation.py`: keep
...
- [x] `tests/integration/test_proxy_transient_retry.py`: A 429 -> B 429 -> C 200
...
- [x] Add product-path walk, exhaustion, and owner-bound coverage for both
...
- [x] `tests/unit/test_streaming_retry_virtual_time.py`: the walk adds no wall
  time between attempts and deadline exhaustion mid-walk terminates with
  `upstream_request_timeout`, not a 429.
- [x] `tests/integration/test_exhaustion_probe_integration.py`: the probe is
  invoked at most once per request from the terminal path.
- [x] `tests/simulation/test_proxy_turn_lifecycle_property.py`: for any sequence
...
- [x] Confirm the `[settings_fields]` ratchet does not move and no new
  `CODEX_LB_*` name is introduced.
- [x] `openspec validate walk-account-pool-before-surfacing-429 --strict`,

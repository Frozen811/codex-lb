...
class ApiKeysService:
    ...
    async def reconcile_usage_reservation(
        self,
        reservation_id: str,
        *,
        ...
        for attempt in range(_SQLITE_BUSY_RETRY_ATTEMPTS):
            try:
                await self._reconcile_usage_reservation_once(
                    reservation_id,
                    request_model=request_model,
                    request_service_tier=request_service_tier,
                    request_usage_budget=request_usage_budget,
                ...
                if not await should_retry_after_sqlite_lock(
                    exc,
                    what="reconcile_usage_reservation",
                    attempt=attempt,
                    max_attempts=_SQLITE_BUSY_RETRY_ATTEMPTS,
                    base_delay_seconds=_SQLITE_BUSY_RETRY_BASE_SECONDS,
                ):
    ...
    async def _reconcile_usage_reservation_once(
        self,
        reservation_id: str,
        *,
        request_model: str | None,
        ...
        async with sqlite_writer_section():
            ...
            row = _ensure_valid_api_key_row(await self._repository.get_for_limit_enforcement(reservation.api_key_id))
            if row.expires_at is not None and row.expires_at < now:
                raise ApiKeyInvalidError("API key has expired")
            limits_reset = await _lazy_reset_expired_limits(self._repository, row.limits, now=now)
            refreshed = (
                _ensure_valid_api_key_row(await self._repository.get_for_limit_enforcement(reservation.api_key_id))
                if limits_reset
                else row
            )
            if refreshed.expires_at is not None and refreshed.expires_at < now:
            ...
            normalized_usage_budget = _normalize_request_usage_budget(request_usage_budget)
            model = request_model or reservation.model
            existing_items_by_limit_id = {item.limit_id: item for item in reservation.items}

            plan: list[tuple[ApiKeyLimit, UsageReservationItemData | None, int, int]] = []
            for limit in refreshed.limits:
                if not _limit_applies_for_request(limit, request_model=model):
                    continue
                target_budget = _reserve_budget_for_limit_type(
                    limit.limit_type,
                    request_model=model,
                    request_service_tier=request_service_tier,
                    request_usage_budget=normalized_usage_budget,
                )
                existing_item = existing_items_by_limit_id.get(limit.id)
                old_reserved_delta = existing_item.reserved_delta if existing_item is not None else 0
                prior_usage = limit.current_value - old_reserved_delta
                remaining = limit.max_value - prior_usage
                if target_budget > remaining:
                    raise _rate_limit_exceeded_error(limit)
                plan.append((limit, existing_item, target_budget, old_reserved_delta))

            try:
                for limit, existing_item, target_budget, old_reserved_delta in plan:
                    delta = target_budget - old_reserved_delta
                    if delta > 0:
                        result = await self._repository.try_reserve_usage(
                            limit.id,
                            delta=delta,
                    ...
                    if existing_item is not None:
                        if delta != 0:
                            await self._repository.update_reservation_item_reserved_delta(
                                reservation_id,
                                limit.id,
                                reserved_delta=target_budget,
                                expected_reset_at=limit.reset_at,
                            )
    ...
    async def get_key_trends(self, key_id: str, days: int = _SPARKLINE_DAYS) -> ApiKeyTrendsData | None:
        row = await self._repository.get_by_id(key_id)
        if row is None:
            return None
        now = utcnow()
    ...
    async def get_key_usage(self, key_id: str, days: int = 7) -> ApiKeyUsageWindowData | None:
        row = await self._repository.get_by_id(key_id)
        if row is None:
            return None
        now = utcnow()
    ...
    async def get_key_usage_7d(self, key_id: str) -> ApiKeyUsage7DayData | None:
        return await self.get_key_usage(key_id, days=7)
...
ApiKeyUsage7DayData = ApiKeyUsageWindowData
...
def _rate_limit_exceeded_error(limit: ApiKeyLimit) -> ApiKeyRateLimitExceededError:
    limit_type_str = getattr(limit.limit_type, "value", limit.limit_type)
    limit_window_str = getattr(limit.limit_window, "value", limit.limit_window)
    return ApiKeyRateLimitExceededError(
        message=f"API key {limit_type_str} {limit_window_str} limit exceeded"

...
def _select_account_preferring_budget_safe(
    ...
    *,
    ...
    if routing_strategy in ("sequential_drain", "reset_drain", "single_account"):
        best_health_drain_states = _best_health_tier_states(state_list)
        budget_safe_states = [
            state
            for state in best_health_drain_states
            if state.routing_policy != ROUTING_POLICY_PRESERVE and not state_budget_threshold(state)
        ]
        return select_account(
            budget_safe_states or best_health_drain_states,
            prefer_earlier_reset=prefer_earlier_reset,
            prefer_earlier_reset_window=prefer_earlier_reset_window,
            routing_strategy=routing_strategy,
            allow_backoff_fallback=allow_backoff_fallback,
    ...
    preferred_states = [
        state
        for state in best_health_states
        if state.routing_policy != ROUTING_POLICY_PRESERVE and not state_budget_threshold(state)
    ]
    if preferred_states:
        selection_pool = preferred_states if len(preferred_states) != len(best_health_states) else best_health_states
        ...
        if preferred.account is not None:
            return preferred
        if len(preferred_states) == len(best_health_states):
            return preferred

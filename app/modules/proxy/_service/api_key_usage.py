...
class _ApiKeyUsageMixin:
    ...
    async def _reconcile_websocket_request_state_reservation(
        self,
        ...
        *,
        ...
        with anyio.CancelScope(shield=True):
            async with proxy._repo_factory() as repos:
                service = _service_api_keys_service()(repos.api_keys)
                try:
                    await service.reconcile_usage_reservation(
                        request_state.api_key_reservation.reservation_id,
                        request_model=request_state.model,
                        request_service_tier=request_state.service_tier,
                        request_usage_budget=request_usage_budget,
                    )
                except ApiKeyRateLimitExceededError as exc:
                    message = f"{exc}. Usage resets at {exc.reset_at.isoformat()}Z."

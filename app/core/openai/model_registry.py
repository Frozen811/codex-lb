...
# Bootstrap slugs that upstream never lists in a model catalog: they route an
# additional-quota allowance (e.g. Luna Reserve, gpt-5.3-codex-spark). Their
# absence from an authoritative catalog is not evidence of withdrawal, so they
# are never suppressed and always served from the bootstrap floor. Kept
# lowercase to match the normalized-slug comparisons in the accessor methods.
_QUOTA_ONLY_BOOTSTRAP_SLUGS: frozenset[str] = frozenset({"gpt-reserve", "gpt-5.3-codex-spark"})

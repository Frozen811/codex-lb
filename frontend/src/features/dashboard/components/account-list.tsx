...
function accountQuotaLabels(account: AccountSummary) {
  ...
  const monthlyOnly =
    ...
    (isPrimaryMonthly && account.windowMinutesSecondary == null);

  if (monthlyOnly) {
    const pct = account.usage?.monthlyRemainingPercent ?? account.usage?.primaryRemainingPercent ?? null;
    const reset = account.resetAtMonthly ?? account.resetAtPrimary;
    return [
      quotaLabel("Monthly", pct, reset),
    ];
  }
  ...
  const primaryLabel = isPrimaryMonthly ? "Monthly" : "5h";
  return [
    quotaLabel(primaryLabel, account.usage?.primaryRemainingPercent ?? null, account.resetAtPrimary),
    quotaLabel("Weekly", account.usage?.secondaryRemainingPercent ?? null, account.resetAtSecondary),
  ];
}
